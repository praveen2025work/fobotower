...
async def test_a_book_reports_its_desk_and_entity(client):
    body = (await client.get("/api/books/APAC-CASH-01")).json()
    assert body["book_ref"] == "APAC-CASH-01"
...
async def test_a_book_carrying_two_breaks_shows_both(client):
    """APAC-CASH-09 has two breaks today; the drawer must not collapse them."""
    body = (await client.get("/api/books/APAC-CASH-09")).json()
