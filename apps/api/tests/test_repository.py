...
async def test_resolves_a_book():
    async with get_session() as s:
        ...
        got = await repo.resolve_book("PRIME-MB-01", date(2026, 8, 3), FO)
        assert got == "book:PRIME-MB-01"
...
async def test_a_caller_outside_the_entity_scope_cannot_resolve():
    ...
    async with get_session() as s:
        ...
        repo = GraphRepository(s)
        with pytest.raises(UnresolvedBook):
            await repo.resolve_book("PRIME-MB-01", date(2026, 8, 3), OUTSIDER)


async def test_as_of_returns_the_hierarchy_in_force_on_that_date():
    """PRIME-MB-05 moved from APAC-TREASURY to APAC-CASH on 2026-07-01."""
    async with get_session() as s:
        await load_all(s)
        repo = GraphRepository(s)
        before = await repo.book_context("book:PRIME-MB-05", date(2026, 6, 15), FO)
        after = await repo.book_context("book:PRIME-MB-05", date(2026, 8, 3), FO)
...
async def test_lineage_is_entitlement_filtered_and_depth_bounded():
    async with get_session() as s:
        ...
        visible = await repo.lineage(
            "book:PRIME-MB-01", "BELONGS_TO", 4, date(2026, 8, 3), FO
        )
        assert [n["natural_key"] for n in visible] == ["APAC-CASH"]

        blocked = await repo.lineage(
            "book:PRIME-MB-01", "BELONGS_TO", 4, date(2026, 8, 3), OUTSIDER
        )
        assert blocked == []
...
async def test_similar_breaks_returns_priors_for_that_book_only():
    ...
    async with get_session() as s:
        ...
        priors = await repo.similar_breaks(
            "book:PRIME-MB-01", "CASH", date(2026, 8, 3), 180, FO
        )
        assert len(priors) > 0
        assert all(p["pattern_code"] == "P-204" for p in priors)
        assert all(p["book_id"] == "book:PRIME-MB-01" for p in priors)
...
async def test_similar_breaks_respects_the_lookback_window():
    async with get_session() as s:
        ...
        priors = await repo.similar_breaks(
            "book:PRIME-MB-01", "CASH", date(2026, 8, 3), 10, FO
        )
        assert priors == []
    ...
    async with get_session() as s:
        ...
        for i in range(1, 13):
            for p in await repo.similar_breaks(
                f"book:PRIME-MB-{i:02d}", "CASH", date(2026, 8, 3), 180, FO
            ):
                seen[p["break_id"]] = p
        assert len(seen) == 42
